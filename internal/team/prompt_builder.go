...
func (p *promptBuilder) Build(slug string) string {
	...
	if p.isOneOnOne() {
		...
		if markdownMemory {
			sb.WriteString("Markdown wiki memory is active in this 1:1. When the work would be clearer as a diagram, report, comparison grid, code explainer, or PR review, build a static OpenUI artifact with visual_artifact_create — the OpenUI document IS the deliverable, so leave source_path empty (see the OPENUI ARTIFACT RULE below). Include visual-artifact:ra_... on its own line when you reference it in chat.\n\n")
	...
	if slug == lead {
		...
		sb.WriteString("All team_*, human_*, and mcp__wuphf-office__* tools listed above are registered for this session. claude-code defers their schemas behind a built-in ToolSearch tool; if the runtime injects a \"call ToolSearch with select:<name> first\" reminder, do it ONCE at the very start of your turn, in a single ToolSearch call. Load ONLY the schemas you actually plan to use this turn — for a typical answer that is team_broadcast (and maybe human_message); add visual_artifact_create ONLY when the OpenUI artifact rule below actually fires. Do NOT preload team_wiki_write unless the human explicitly asked for that exact action; it is banned for unsolicited use. (team_task is exempt — Rule Zero requires team_task action=create first in the normal flow. The non-trivial Issue-spec flow below is the one exception: create the visual artifact first so its returned id can be included in the task details.) Load team_task whenever you will create or comment on an Issue. Then proceed with the real work in the same assistant response. Never call ToolSearch a second time in the same turn.\n")
		...
		sb.WriteString("Broadcast budget: AT MOST one team_broadcast per turn for a normal answer; AT MOST two when the OpenUI artifact rule below fires (gist + link card). No plan/preamble broadcasts. Never re-post the same content in different wording.\n\n")
		sb.WriteString(secretHandlingPromptRule())
		if markdownMemory {
		...
		sb.WriteString("STYLE: Be concise, delegate, short lively messages. Use compact markdown for simple chat structure; for dense or visual outputs, create an OpenUI artifact instead of leaving the human with a long markdown wall.\n")
		if markdownMemory {
		...
		sb.WriteString("STYLE: Be concise, stay in lane, short lively messages. Use compact markdown for simple chat structure; for dense or visual outputs, create an OpenUI artifact instead of leaving the human with a long markdown wall.\n")
		sb.WriteString("Never launch another WUPHF office from inside your turn (`wuphf`, `./wuphf`, `/reset`, or a new browser instance). The office is already running; inspect the current repo and UI instead.\n")
	}
...
func markdownKnowledgeToolBlock() string {
	return "- visual_artifact_create: When the answer genuinely benefits from a dense visual document — complex specs, implementation plans, code explainers, PR reviews, comparison grids, diagrams, or reports — produce a static OpenUI document with this tool. The OpenUI document IS the deliverable: leave source_path empty (see the OPENUI ARTIFACT RULE below for the selectivity test). Include visual-artifact:ra_... on its own line in your chat summary so the UI renders an artifact card.\n" +
		"- visual_artifact_list / visual_artifact_read / visual_artifact_promote: Reuse, inspect, and promote existing visual artifacts into wiki visual views.\n" +
...
// visualArtifactForcingBlock returns the selectivity decision tree for OpenUI
...
func visualArtifactForcingBlock() string {
	return "OPENUI ARTIFACT RULE — selectivity, not reflex:\n" +
		"The visual_artifact_create tool produces a structured static OpenUI document. It is NOT the default answer format. Use it only when the answer genuinely benefits from a rich visual document. For most chat — questions, status updates, short answers, coordination, confirmations, conversational explanations — answer in plain text in the channel and STOP.\n\n" +
		"USE an OpenUI artifact ONLY when ALL THREE of these are true:\n" +
		...
		"DO NOT use an OpenUI artifact when ANY of these are true:\n" +
		...
		"When OpenUI is not warranted: answer in chat as plain text in ONE team_broadcast (or human_message in a 1:1). Do not call visual_artifact_create.\n\n" +
		"WHEN OPENUI IS WARRANTED — follow the exact component contract in the tool schema. Emit OpenUI Lang only: no HTML, Markdown, URLs, images, forms, actions, queries, mutations, or state. Keep tables and charts concise.\n\n" +
		"ATOMIC-TURN RULE (only when OpenUI is warranted): all three tool calls happen in the SAME assistant response.\n" +
		...
		"  2. visual_artifact_create — the complete OpenUI Lang document. Capture the returned ra_... id. Leave source_path empty; the OpenUI document is the article, not a companion to a markdown file.\n" +
		...
		"When you create an Issue whose spec is non-trivial — a plan, RFC, design doc, proposal, roadmap, architecture write-up, playbook, multi-step approach, or any spec that would naturally run beyond ~200 words of structured prose with headings/sections — use this exact same-turn sequence: (1) visual_artifact_create with the full OpenUI spec; (2) capture its returned ra_... id; (3) team_task action=create with `visual-artifact:ra_<id>` on its own line inside the `details` field; (4) send the one closing response with the task and artifact references. This is the ONLY exception to the team_task-first rule because the task cannot reference an id that does not exist yet. Keep `details` short (1-3 sentences); the article body lives in the artifact. If the Issue already exists, use team_task action=comment to attach the returned artifact id instead of creating another Issue. Skip the artifact for trivial Issues.\n\n" +
