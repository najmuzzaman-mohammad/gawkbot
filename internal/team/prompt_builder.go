...
func (p *promptBuilder) Build(slug string) string {
	...
	if slug == lead {
		...
		sb.WriteString("All team_*, human_*, and mcp__wuphf-office__* tools listed above are registered for this session. claude-code defers their schemas behind a built-in ToolSearch tool; if the runtime injects a \"call ToolSearch with select:<name> first\" reminder, do it ONCE at the very start of your turn, in a single ToolSearch call. Load ONLY the schemas you actually plan to use this turn — for a typical answer that is team_broadcast (and maybe human_message); add visual_artifact_create ONLY when the OpenUI artifact rule below actually fires. Do NOT preload team_wiki_write unless the human explicitly asked for that exact action; it is banned for unsolicited use. (team_task is exempt — Rule Zero requires team_task action=create as your FIRST tool call on any work-shaped request, so load team_task whenever you will create or comment on an Issue.) Then proceed with the real work in the same assistant response. Never call ToolSearch a second time in the same turn.\n")
...
func visualArtifactForcingBlock() string {
	return "OPENUI ARTIFACT RULE — selectivity, not reflex:\n" +
		...
		"When you call team_task action=create for an Issue whose spec is non-trivial — a plan, RFC, design doc, proposal, roadmap, architecture write-up, playbook, multi-step approach, or any spec that would naturally run beyond ~200 words of structured prose with headings/sections — you MUST also call visual_artifact_create in the SAME assistant response with the full OpenUI spec, and include `visual-artifact:ra_<id>` on its own line inside the `details` field you pass to team_task. Keep `details` short (1-3 sentences); the article body lives in the artifact. Skip the artifact for trivial Issues.\n\n" +
