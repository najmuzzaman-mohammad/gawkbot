...
// TestBotActivitySnapshotKindJSONRoundTrip pins the wire contract for the
// new Kind field on botActivitySnapshot. Three things matter to lane B/C
// (frontend) consumers:
//
...
func TestBotActivitySnapshotKindJSONRoundTrip(t *testing.T) {
	...
	for _, kind := range []string{"routine", "milestone", "stuck"} {
		...
		t.Run("round trip "+kind, func(t *testing.T) {
			...
			var decoded botActivitySnapshot
