...
//  3. A runnable service dir exists (WUPHF_AGENT_DIR, or bot/ next to the
...
func (b *Broker) startBotServiceSupervisor(ctx context.Context) {
	if strings.TrimSpace(os.Getenv("WUPHF_AGENT_URL")) != "" {
		log.Printf("bot-service: WUPHF_AGENT_URL set — externally managed, not spawning")
		return
	}
	...
	go func() {
		for {
			...
			if dir == "" || bunErr != nil {
				if dir == "" {
					log.Printf("bot-service: no service dir found (set WUPHF_AGENT_DIR or run from a checkout) — routines need it to fire")
				} else {
...
// resolveBotServiceDir finds a runnable bot/ package: WUPHF_AGENT_DIR
