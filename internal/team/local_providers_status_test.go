...
func TestComputeLocalProviderStatuses_AllInstalledAndReachable(t *testing.T) {
	...
	for _, k := range []string{provider.KindAtlasCloud, provider.KindMLXLM, provider.KindOllama, provider.KindExo, provider.KindHermesBot, provider.KindOpenclawHTTP} {
		s, ok := gotByKind[k]
		if !ok {
...
func TestComputeLocalProviderStatuses_LinuxHidesMLXButShowsOthers(t *testing.T) {
	...
	for _, s := range got {
		...
		case provider.KindAtlasCloud, provider.KindOllama, provider.KindExo, provider.KindHermesBot, provider.KindOpenclawHTTP:
			if !s.PlatformSupported {
