...
// Fields mirror the record written by BotLoop.logToolExecution in loop.go.
...
// DefaultTaskLogRoot returns the directory where BotLoop writes task logs.
...
func summarizeTaskLog(path, taskID string) TaskLogSummary {
	...
	for scanner.Scan() {
		...
		s.ToolCallCount++
		if s.BotSlug == "" && entry.BotSlug != "" {
			s.BotSlug = entry.BotSlug
		}
