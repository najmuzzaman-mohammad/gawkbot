...
func newTestLoop(t *testing.T, streamFn StreamFn) (*BotLoop, string) {
	t.Helper()
	dir := t.TempDir()
	...
	loop := NewBotLoop(config, tools, sessions, queues, streamFn, nil)
	return loop, dir
}

func TestStreamLLMReceiveStopsOnCancel(t *testing.T) {
	streamStarted := make(chan struct{}, 1)
	streamFn := func(msgs []Message, tools []BotTool) <-chan StreamChunk {
		select {
...
func TestFullTickCycle(t *testing.T) {
	...
	var phaseChanges []BotPhase
	loop.On(EventPhaseChange, func(args ...any) {
		mu.Lock()
		defer mu.Unlock()
		if len(args) >= 2 {
			if p, ok := args[1].(BotPhase); ok {
...
func TestBuildContextDrainsHumanBeforeFollowUp(t *testing.T) {
	...
	// Simulate a busy bot already working on a bot-originated follow-up
	...
	loop.queues.FollowUp("test-agent", "previously queued bot task")
	...
	for _, e := range entries {
		...
		case e.Type == "user" && e.Content == "[HUMAN] wait, what are you doing?":
			sawHuman = true
		case e.Type == "user" && e.Content == "previously queued bot task":
			sawFollowUp = true
		}
	}
	if !sawDirective {
	...
	if !loop.queues.HasFollowUp("test-agent") {
		t.Error("follow-up queue should still hold the deferred bot task")
	}
}
...
func TestCredibilityRecordOnDone(t *testing.T) {
	...
	loop := NewBotLoop(config, tools, sessions, queues, mockStreamFn("ok"), tracker)
