...
func TestExtractDelegations(t *testing.T) {
	...
	if delegations[0].BotSlug != "research" {
		t.Errorf("expected first delegation to 'research', got '%s'", delegations[0].BotSlug)
	}
	if delegations[1].BotSlug != "content" {
		t.Errorf("expected second delegation to 'content', got '%s'", delegations[1].BotSlug)
	}
}
...
func TestExtractDelegationsIgnoresUnknownSlugs(t *testing.T) {
	...
	if delegations[0].BotSlug != "research" {
		t.Errorf("expected 'research', got '%s'", delegations[0].BotSlug)
	}
}
...
func TestExtractDelegationsSentenceContext(t *testing.T) {
	...
	for _, d := range delegations {
		if d.Task == "" {
			t.Errorf("delegation to %s has empty task", d.BotSlug)
		}
	}
}
...
func TestApplyLimitEnforced(t *testing.T) {
	...
	delegations := []Delegation{
		{BotSlug: "fe", Task: "build UI"},
		{BotSlug: "be", Task: "build API"},
		{BotSlug: "qa", Task: "write tests"},
	}
	immediate, queued := d.ApplyLimit(delegations)
	if len(immediate) != 1 {
		t.Fatalf("expected 1 immediate, got %d", len(immediate))
	}
	if immediate[0].BotSlug != "fe" {
		t.Errorf("expected immediate[0]='fe', got '%s'", immediate[0].BotSlug)
	}
	if len(queued) != 2 {
		t.Fatalf("expected 2 queued, got %d", len(queued))
	}
	if queued[0].BotSlug != "be" {
		t.Errorf("expected queued[0]='be', got '%s'", queued[0].BotSlug)
	}
	if queued[1].BotSlug != "qa" {
		t.Errorf("expected queued[1]='qa', got '%s'", queued[1].BotSlug)
	}
}
