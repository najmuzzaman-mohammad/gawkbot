/**
 * Bot event timer — pure pill-state derivation + a single 1Hz scheduler.
 *
 * Eng decision C2: one shared `setInterval(1000)` walks all snapshots; rows
 * subscribe to a derived slice via shallow-equality selectors. Per-bot
 * timers are not used.
 *
 * The 1Hz scheduler MUST clean up on unmount or it will keep ticking after
 * BotList is gone (dev hot-reload, route changes, multi-tab) — flagged as
...
export interface PillStateInput {
  /** Wall-clock timestamp (ms) of the most recent event for this bot. */
  lastEventMs: number;
