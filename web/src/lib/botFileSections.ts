/**
 * botFileSections — parse the per-bot instruction files (SOUL / IDENTITY /
 ...
 * internal/team/agent_files.go (renderBotSoul/Operations/Tools). Keep them in
 * sync so a freshly-seeded file maps cleanly onto its schema blocks.
 */

export interface BotFileSection {
...
export const AGENT_FILE_SECTION_SCHEMA: Record<string, SectionSchema[]> = {
  SOUL: [
    {
      heading: "Who you are",
      hint: "The bot's core identity, in a line or two.",
    },
    {
      heading: "Values",
  ...
  OPERATIONS: [
    {
      ...
      hint: "The day-to-day operating procedure for this bot.",
    },
    {
      heading: "Escalation",
  ...
  TOOLS: [
    { heading: "Available tools", hint: "The tools this bot may use." },
    {
      heading: "Notes",
 ...
 */
export function parseBotFile(markdown: string): ParsedBotFile {
  const lines = (markdown ?? "").split("\n");
  let title = "";
  const preambleLines: string[] = [];
  const sections: BotFileSection[] = [];
...
export function serializeBotFile(parsed: {
  title: string;
  preamble: string;
  sections: BotFileSection[];
}): string {
