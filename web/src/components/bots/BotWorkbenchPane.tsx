...
interface BotWorkbenchPaneProps {
  agentSlug: string;
}
...
export function BotWorkbenchPane({ agentSlug }: BotWorkbenchPaneProps) {
  const { data: tasks = [] } = useOfficeTasks();
  // Office-wide run log is not server-side filterable by bot, so we fetch a
  ...
  return (
    <div className="bot-workbench-pane" data-testid="bot-workbench-pane">
      ...
      {/* keepMounted: collapsing this section must not tear down the
          EventSource owned by useBotStream — the live subscription and
      ...
      <CollapsibleSection
        ...
        keepMounted={true}
      >
        <BotStreamSection slug={agentSlug} />
      </CollapsibleSection>
...
interface BotStreamSectionProps {
  slug: string;
}

function BotStreamSection({ slug }: BotStreamSectionProps) {
