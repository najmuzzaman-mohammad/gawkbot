...
vi.mock("../../api/botFiles", async () => {
  ...
  return { ...actual, writeBotFile: writeBotFileMock };
});
...
function makeData(sha: string): BotFileResponse {
  return {
...
describe("<AgentFileBlockEditor>", () => {
  ...
  it("saves against the SHA snapshotted at edit-open, not a SHA that advanced mid-edit", async () => {
    ...
    const arg = writeBotFileMock.mock.calls[0][0] as { expectedSha: string };
