/**
 * Tasks tab — owner-filtered board showing tasks assigned to this bot.
...
const BotTaskCard = memo(function TaskCard({ task }: { task: Task }) {
...
export function TasksTab({ agentSlug }: TasksTabProps) {
  ...
  const botTasks = allTasks.filter((t) => t.owner === agentSlug);

  const columns: StageColumn[] = STAGE_ORDER.filter(
    (stage) => stage !== "scheduled",
  ).map((stage) => ({
    stage,
    tasks: botTasks.filter(
