...
describe("<AgentWizard>", () => {
  ...
  it("closes on Escape from the window while open", () => {
    const onClose = vi.fn();
    render(wrap(<BotWizard open={true} onClose={onClose} />));
  ...
  it("does not close on Escape while creating a bot", async () => {
  ...
  it("sends the soul field as `personality` in the create body", async () => {
    ...
    render(
      wrap(<BotWizard open={true} onClose={vi.fn()} onCreated={vi.fn()} />),
    );
