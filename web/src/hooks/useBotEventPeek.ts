 ...
 * Used by BotEventPill's chevron for aria-expanded without subscribing
...
export function useBotEventPeek(
  ...
  const current = useAppStore((s) => s.botActivitySnapshots[slug]);
  ...
  const historyOrUndef = useAppStore((s) => s.botActivityHistory[slug]);
