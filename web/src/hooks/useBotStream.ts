...
// appendStreamLine is the pure update used by useBotStream's
...
// botStreamURL builds the SSE URL for a bot's live output, optionally
...
export function botStreamURL(slug: string, taskId: string | null): string {
...
export function useBotStream(
  ...
  // url overrides the default /bot-stream/{slug}?task= endpoint. The
  ...
  // A single-turn view (the bot's Live Stream tab) wants the connection to
  ...
  useEffect(() => {
    ...
    const url = urlOverride ?? botStreamURL(slug, taskId);
    ...
    source.onmessage = (e) => {
      ...
      //      stream view for a bot that just went idle.
