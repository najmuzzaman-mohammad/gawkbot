...
import { __internal, useBotEventPeek, usePeekIsOpen } from "./useBotEventPeek";
...
const FAST: Parameters<typeof useBotEventPeek>[1] = {
  hoverIntentMs: 300,
  closeGraceMs: 80,
  longPressMs: 500,
};
...
describe("initial state", () => {
  it("isOpen is false before any interaction", () => {
    const { result } = renderHook(() => useBotEventPeek("tess", FAST));
...
describe("long-press touch", () => {
  it("opens after 500ms touchstart without move or end", () => {
    const { result } = renderHook(() => useBotEventPeek("ava", FAST));
    act(() => {
...
describe("single-instance: opening one slug closes another", () => {
  it("opening slug B leaves slug A closed", () => {
    const hookA = renderHook(() => useBotEventPeek("tess", FAST));
    const hookB = renderHook(() => useBotEventPeek("ava", FAST));
...
describe("unmount cleanup", () => {
  it("cancels pending hover-open timer on unmount without error", () => {
    const { result, unmount } = renderHook(() => useBotEventPeek("tess", FAST));
    act(() => {
      result.current.hoverHandlers.onMouseEnter();
    });
  ...
  it("cancels pending long-press timer on unmount without error", () => {
    const { result, unmount } = renderHook(() => useBotEventPeek("ava", FAST));
    act(() => {
      result.current.longPressHandlers.onTouchStart();
    });
    unmount();
...
describe("snapshot data from store", () => {
  it("exposes current snapshot when one exists for the slug", () => {
    ...
    useAppStore.setState({
      botActivitySnapshots: { tess: snap },
    });
  ...
  it("exposes history array for the slug", () => {
    ...
    useAppStore.setState({
      botActivitySnapshots: { tess: snap2 },
      botActivityHistory: { tess: [snap1] },
    });
